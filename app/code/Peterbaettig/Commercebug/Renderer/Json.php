...
* All rights reserved
*/
namespace Peterbaettig\Commercebug\Renderer;
use Peterbaettig\Commercebug\Model\All;
use ReflectionClass;
class Json
{
    ...
    */    
    public function clean($data)
    {
        ...
        foreach($data as $key=>$value)
        {
            if($key === 'invoked_observers')
            {
                $data[$key] = array_filter($value, function($val){
                    return strpos($val['instance'], 'Peterbaettig\\Commercebug') === false;
                });
            }
            ...
            if(in_array($key,['models','collections']) && is_array($value))
            {
                ...
                foreach($data[$key] as $index=>$object)
                {
                    ...
                    $new[get_class($object)] = ['file'=>$r->getFilename()];
                }
                if(count($new))
                {
                    $data[$key] = $new;
        ...
        {
            $data['layouts'] = [
                'graph'=> \Peterbaettig\Commercebug\Plugins\MagentoFrameworkViewLayout::renderGraph(),
