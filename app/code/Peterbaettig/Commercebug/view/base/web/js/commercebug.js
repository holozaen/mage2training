        ...
        var report = function(element, indent, nodes, inferredParent)
        {
            ...
            if(element.bodyTmpl){
                addNode(getUniqueId() + element.name + '_bodytemplate', uniqueGeneratedIdForTree,
                    '<strong>Knockout.js bodyTmpl:</strong> '   + 
                        element.bodyTmpl + ' ' +
                        getKnockoutJsHref(element.bodyTmpl), nodes);
                logLocal(indent + '    - bodyTmpl: ' + element.bodyTmpl);
            }
    
            if(element.headerTmpl){
                addNode(getUniqueId() + element.name + '_headertemplate', uniqueGeneratedIdForTree,
                    '<strong>Knockout.js headerTmpl:</strong> ' +                     
                        element.headerTmpl + ' ' +
                        getKnockoutJsHref(element.headerTmpl), nodes);
                logLocal(indent + '    - headerTmpl: ' + element.headerTmpl);    
            }
